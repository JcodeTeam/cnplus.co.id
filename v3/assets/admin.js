(() => {
  ...
  const collapseLinks = () => getLinks().forEach(link => {
    link.classList.remove('gap-3', 'px-3');
    link.classList.add('gap-0', 'px-0', 'justify-center');
  });

  const expandLinks = () => getLinks().forEach(link => {
    link.classList.remove('gap-0', 'px-0', 'justify-center');
    link.classList.add('gap-3', 'px-3');
  });

  const hideMenuLabels = () => getLabels().forEach(l => l.classList.add('hidden'));
  const showMenuLabels = () => getLabels().forEach(l => l.classList.remove('hidden'));
  ...
  const hasFilterUI = !!(tbody && qInput && btnDropdown && dropdown && listInd && listProd);
...
<tr class="hover:bg-zinc-50">
  ...
  <td class="py-3 px-4 text-center whitespace-nowrap">${markify(created, terms)}</td>
    ...
    const selected = () => {
      const inds = $$('input[type="checkbox"]:checked', listInd).map(i => i.value);
      const prods = $$( 'input[type="checkbox"]:checked', listProd).map(i => i.value);
      return { inds, prods };
    };
  ...
  <span>${esc(v)}</span>
    ...
    const filterLocal = q => {
      ...
      const rows = allRows.filter(r => {
        ...
        const hay = [
          r.fullname, r.email, r.company, r.industry, r.phone, r.product, r.description,
    ...
    btnAllInd?.addEventListener('click', () => {
      $$('input[type="checkbox"]:checked', listInd).forEach(i => i.checked = false);
      runFilter();
    });
    btnAllProd?.addEventListener('click', () => {
      $$('input[type="checkbox"]:checked', listProd).forEach(i => i.checked = false);
      runFilter();
    });
    qInput.addEventListener('input', onType);
    qInput.addEventListener('keydown', e => {
      if (e.key === 'Escape') { qInput.value = ''; runFilter(); }
    });
  ...
  const settingsToggles = () => {
    ...
    buttons.forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try {
          const res = await fetch('../api/update_notify.php', { method: 'POST', body: formData });
          ...
          if (data && data.success) {
            ...
            if (next === 1) {
              btn.className = 'toggle-btn inline-flex items-center h-8 px-3 rounded-xl text-xs font-medium border transition-colors bg-emerald-600 border-emerald-700 text-white hover:bg-emerald-700';
